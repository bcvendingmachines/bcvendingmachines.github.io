...
** For the full stack to work locally, use the correct configs in `configs.ts` in `src\app\service`. Changes to the configs shouldn't be pushed. `config.ts` is in the .gitignore file.
